...
@section('content')
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-3xl mx-auto">
            <h1 class="text-3xl font-bold text-gray-800 mb-6">Edit Pricing Plan</h1>

            <form action="{{ route('pricings.update', $pricing) }}" method="POST" class="bg-white rounded-lg shadow-md p-6">
                @csrf
                @method('PUT')
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $pricing->name) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <select name="period" id="period"
                            ...
                            <option value="month" {{ old('period', $pricing->period) == 'month' ? 'selected' : '' }}>
                                Monthly</option>
                            <option value="year" {{ old('period', $pricing->period) == 'year' ? 'selected' : '' }}>Yearly
                            </option>
                            <option value="one-time" {{ old('period', $pricing->period) == 'one-time' ? 'selected' : '' }}>
                                One-time</option>
                        </select>
                        @error('period')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description" id="description" rows="3"
                        class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $pricing->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <div id="features-container">
                        @foreach (old('features', $pricing->features ?? []) as $index => $feature)
